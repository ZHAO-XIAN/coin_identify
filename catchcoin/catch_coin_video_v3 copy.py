...
# 讀取影片
cap = cv2.VideoCapture('../video/transfer_video_v3.mp4')  # 請確保 'input_video.mp4' 是您的影片檔案名稱
...
out = cv2.VideoWriter('catch_coin_video_v3.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_width, frame_height))

# 定義 ROI 範圍 (左上角 x, y 以及寬度和高度)
roi_x, roi_y, roi_w, roi_h = 100, 120, 600, 400

paused = False  # 控制播放/暫停的變數

# 用來追蹤硬幣的ID和其位置
tracked_coins = {}  # key: 圓心位置，value: 代號
previous_positions = {}  # 用來記錄上一幀的位置，用於寬恕

# 用來追蹤硬幣的位置變化
def find_matching_coin(new_center, threshold=100):
    """搜尋是否有與新檢測到的圓心相近的硬幣"""
...
while cap.isOpened():
    ...
    ret, blurred_threshold = cv2.threshold(gray, 100, 255, cv2.THRESH_BINARY)
    
    kernel = np.ones((11, 11), np.uint8) #結構元素(kernel)
    closing = cv2.morphologyEx(blurred_threshold, cv2.MORPH_CLOSE, kernel)
    blurred_canny = cv2.Canny(gray_blurred, 100, 180)
    ...
    if circles is not None:
        ...
        for idx, i in enumerate(circles[0, :]):
            ...
            if matching_key is not None:
                ...
                previous_positions[label] = circle_center  # 更新上一幀位置
            else:
                # 如果是新硬幣，給它一個新的代號
                label = f"C{len(tracked_coins) + 1}"
                tracked_coins[circle_center] = (circle_center, label)
                previous_positions[label] = circle_center  # 記錄新硬幣的初始位置
        ...
        # 如果沒有偵測到圓形，使用上一幀的紀錄位置
        for label, prev_center in previous_positions.items():
            if label not in [label for _, (_, label) in tracked_coins.items()]:  # 確保只更新未丟失的硬幣
                # 畫上上一幀的位置
                cv2.circle(output, prev_center, 10, (255, 0, 0), 2)  # 使用不同顏色來表示
                # 標記代號 (C1, C2, C3...)
                cv2.putText(output, label, (prev_center[0] - 20, prev_center[1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
    ...
    cv2.imshow('blurred_threshold', closing)
    cv2.imshow('blurred_canny', blurred_canny)
