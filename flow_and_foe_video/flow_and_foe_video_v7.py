...
if __name__ == "__main__":
    ...
    while cap.isOpened():
        ...
        if flow is not None and magnitude is not None:
            ...
            # 計算FOE座標
            foe_x, foe_y = flow_create_foe.calculate_foe_coordinate(flow, h, w)
            # 標記FOE在畫面上
            flow_create_foe.mark_foe(frame, (foe_x, foe_y))
            # 標記光流流向在畫面上
            flow_image = flow_create_foe.draw_flow(flow, frame.copy(), h, w)
            # 產生深度圖
            z_resized = flow_create_foe.create_depth_image(h, w, flow, foe_x, foe_y)
